    ...
    describe '.robots' do
      let(:robots) do
        VCR.use_cassette 'robots' do
          Net::HTTP.get_response(URI(ErogeRelease::GetchuyaScraping::ROBOTS_TXT_URL)).body
        end
      end
      let(:get_robots) do
    ...
    describe '.parsed_html_for_uri' do
      let(:release_list) do
        VCR.use_cassette 'release_list_201902' do
          ...
          http.get(uri_game_list_201902, Cookie: ErogeRelease::GetchuyaScraping::COOKIE_OPTION)
        end
      end
