...
require './eroge_release/getchuya_scraping/release_list_scraping'
  ...
  describe Games do
    ...
    context 'when not initialize process' do
      ...
      describe '#json_file_path' do
        subject { "#{ErogeRelease::Games::CREATED_PATH}#{games.year}#{games.month}.json" }
      ...
      describe '#csv_file_path' do
        subject { "#{ErogeRelease::Games::CREATED_PATH}#{games.year}#{games.month}.csv" }
      ...
      describe '#create_csv' do
        ...
        it { expect(header).to eq ErogeRelease::Games::HEADER_COLUMNS.to_s }
